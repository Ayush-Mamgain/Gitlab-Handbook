...
import { registerUser, fetchUser, clearAuthError } from '@/store/authSlice';
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result = await dispatch(registerUser({ username: username.trim(), password }));
  ...
  const isValid = username.trim() && password.trim() && confirm.trim() && password === confirm;
    ...
    <div className="min-h-full flex items-center justify-center bg-[#0f0f1a] px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              required
              autoComplete="new-password"
              className={`w-full px-3.5 py-2.5 rounded-xl bg-[#1e1e2e] border text-sm text-white placeholder-slate-600 outline-none transition-all duration-150
                ${confirm && confirm !== password
                  ? 'border-red-500/50 focus:border-red-500/70'
                  : 'border-white/8 focus:border-[#E24329]/50 focus:ring-1 focus:ring-[#E24329]/20'
                }`}
            />
            {confirm && confirm !== password && (
        ...
        <p className="text-center text-xs text-slate-600 mt-6">
          ...
          <Link href="/login" className="text-[#E24329] hover:underline">
            Sign in
          </Link>
        </p>
      </div>
